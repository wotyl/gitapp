	...
	var inputcpe = function(){
			var coll;
			var num = parseInt($('#video_local_search_summary').html().replace("Найдено ","").replace(" видеофайлов","").replace(" видеофайла","").replace(" видеофайл","").replace("Найден ",""));
			if(num > 7){
				coll = "-------";
		    }else{
		    	coll = "Найдено: "+num
		    ...
		    for(var i=0; i<$('#video_rows>div').length; i++){
		    	...
		    	$('body').append('<div class="left_panel"><input class="ID_grup"></input><input class="id_g" type="submit"></input><div class="idG_res"></div></div>');

			}	
			flar_two = false;
			if(ind<arr_search.length){
